...
import SampleApplication.utils.Banana;
import SampleApplication.utils.CubeShaders;
import SampleApplication.utils.Dmsb;
import SampleApplication.utils.Dongmen;
import SampleApplication.utils.LoadingDialogHandler;
import SampleApplication.utils.Majunwu;
...
import SampleApplication.utils.Shuita;
 ...
 */
public class ImageTargetsRendererMJW implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    private static final String LOGTAG = "ImageTargetRendererMJW";

    private final SampleApplicationSession vuforiaAppSession;
    private final WeakReference<ImageTargetsMJW> mActivityRef;
...
//    private Banana mBanana;
    private Majunwu mMajunwu;
//    private Dmsb mDmsb;
//    private Shuita mShuita;
//    private Dongmen mDongmen;
//    private AssetManager assetManager;


    //sw   private static final float BUILDING_SCALE = 0.012f;
//    private static final float BUILDING_SCALE = 12.0f;
    private static final float BUILDING_SCALE = 12.0f;
    ...
    // sw   private static final float OBJECT_SCALE_FLOAT = 0.003f;
    private static final float OBJECT_SCALE_FLOAT = 0.12f;





    static float kOBJECT_SCALE_FLOAT = 0.24f;


//    static  float OBJECT_SCALE_FLOAT = 0.12f;

    public static float getkOBJECT_SCALE_FLOAT() {
        return kOBJECT_SCALE_FLOAT;
    }

    public static void setkOBJECT_SCALE_FLOAT(float kOBJECT_SCALE_FLOAT) {
        ImageTargetsRendererMJW.kOBJECT_SCALE_FLOAT = kOBJECT_SCALE_FLOAT;
    }

    public static float getmAngleY() {
        return mAngleY;
    }

    public static void setmAngleY(float mAngleY) {
        ImageTargetsRendererMJW.mAngleY = mAngleY;
    }

    static float mAngleY = 0.12f;

    public static float getmAngleX() {
        return mAngleX;
    }

    public static void setmAngleX(float mAngleX) {
        ImageTargetsRendererMJW.mAngleX = mAngleX;
    }

    static float mAngleX = 0.12f;
    ...
    ImageTargetsRendererMJW(ImageTargetsMJW activity, SampleApplicationSession session)
    {
    ...
    private void initRendering()
    {
        ...
        if(!mModelIsLoaded) {
             ...
             */
            //mTeapot = new Teapot();
            //mBanana=new Banana(mActivityRef.get().getResources().getAssets());
              mMajunwu=new Majunwu(mActivityRef.get().getResources().getAssets());
            //mShuita=new Shuita(mActivityRef.get().getResources().getAssets());
            // mDongmen=new Dongmen(mActivityRef.get().getResources().getAssets());
//            mDmsb=new Dmsb(mActivityRef.get().getResources().getAssets());
...
//                        : 1;
// 在这里替换target图片
                textureIndex = trackable.getName().equalsIgnoreCase("Weixin") ? 0
                        : 1;
...
//            Matrix.scaleM(modelMatrix, 0, OBJECT_SCALE_FLOAT, OBJECT_SCALE_FLOAT, OBJECT_SCALE_FLOAT);
            Matrix.rotateM(modelMatrix, 0, 90.0f, 90, 0, 0);


            Matrix.translateM(modelMatrix, 0, 0, 0, 0);
            Matrix.scaleM(modelMatrix, 0, kOBJECT_SCALE_FLOAT, kOBJECT_SCALE_FLOAT, kOBJECT_SCALE_FLOAT);
            Matrix.rotateM(modelMatrix,0,mAngleY, 0, 1, 0);//左右滑动让模型左右转动
